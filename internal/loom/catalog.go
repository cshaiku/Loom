...
var Commands = []CommandInfo{
	...
	{"generate:contracts", "generate target contracts", "emit inspectable native behavior, state, action, collection, policy, and accessibility handoff contracts", "generation", AccessConditionalWrite, []string{"--output"}, []string{"contracts"}, []string{"loom generate:contracts <source-file> [--target swiftui|winui3|qt] [--from swiftui|winui3|qt] [--format text|json] [--output path]"}, []string{"loom generate:contracts contentview.swift --target winui3 --json"}},
	{"generate:swiftui", "generate swiftui scaffold", "emit reviewable SwiftUI scaffold from normalized WinUI XAML, Qt, or SwiftUI source", "generation", AccessConditionalWrite, []string{"--output"}, []string{"swiftui"}, []string{"loom generate:swiftui <source-file> [--from winui3|qt|swiftui] [--view-name name] [--format text|json] [--output path]"}, []string{"loom generate:swiftui mainwindow.xaml --view-name MainWindowScaffold"}},
	{"generate:xaml", "generate winui xaml", "emit reviewable WinUI XAML fragments and support guarded owned-region replacement", "generation", AccessConditionalWrite, []string{"--output", "--replace-region"}, []string{"generate"}, []string{"loom generate:xaml <source-file> [--from swiftui|winui3|qt] [--root-view name] [--component name] [--theme-prefix prefix] [--patterns-dir path] [--pattern-comments] [--format text|json] [--output path]", "loom generate:xaml <source-file> --replace-region <xaml-file> [--region-id id] [--init-region] [--overwrite]"}, nil},
