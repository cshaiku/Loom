...
var Commands = []CommandInfo{
	...
	{"graph:components", "graph layout components", "reserved for future go work: discover reachable layout components and custom dependencies", "inspection", AccessConditionalWrite, []string{"--output"}, []string{"graph"}, []string{"loom graph:components <source-file-or-directory> [--root-view name] [--component name] [--format text|json|dot] [--include glob] [--exclude glob]"}, nil},
	...
	{"inspect:errors", "inspect errors", "report source, xaml, manifest, or pattern errors", "inspection", AccessConditionalWrite, []string{"--output"}, []string{"errors"}, []string{"loom inspect:errors <path> [--kind swift|xaml|manifest|patterns] [--root-view name] [--component name] [--format text|json] [--fail-on none|error|warning] [--output path]"}, []string{"loom inspect:errors mainwindow.xaml --kind xaml --format json"}},
	{"inspect:parity", "inspect layout parity", "reserved for future go work: compare source and target layout structure", "inspection", AccessConditionalWrite, []string{"--output"}, []string{"parity"}, []string{"loom inspect:parity <source-file> --xaml <xaml-file> [--root-view name] [--format text|json]"}, nil},
	{"inspect:source", "inspect layout source", "reserved for future go work: extract and report a source layout tree", "inspection", AccessConditionalWrite, []string{"--output"}, []string{"analyze"}, []string{"loom inspect:source <source-file> [--root-view name] [--component name] [--format text|json]"}, nil},
	...
	{"patterns:transfer", "plan pattern transfer", "classify how safely layout patterns transfer between OS UI frameworks", "patterns", AccessConditionalWrite, []string{"--output"}, []string{"transfer"}, []string{"loom patterns:transfer <xaml-file> [--from winui3] [--to swiftui|macos] [--root-view name] [--component name] [--patterns-dir path] [--format text|json] [--output path]"}, []string{"loom patterns:transfer mainwindow.xaml --from winui3 --to macos --format json"}},
